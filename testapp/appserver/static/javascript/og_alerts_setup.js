...
require.config({
    paths: {
        // $SPLUNK_HOME/etc/apps/SPLUNK_APP_NAME/appserver/static/javascript/views/og_alerts_setup_view
        OGSetupView: "../app/" + app_name + "/javascript/views/og_alerts_setup_view",
    },
});
...
require([
    ...
    // Custom files
    "OGSetupView",
], function(Backbone, jquery, OGSetupView) {
    var og_setup_view = new OGSetupView({
        // Sets the element that will be used for rendering
        el: jquery("#main_container"),
    });

    og_setup_view.render();
});